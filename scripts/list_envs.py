...
All the environments are registered in the `yam_ultra_deformable_place` extension. They start
...
import yam_ultra_deformable_place.tasks  # noqa: F401

# PLACEHOLDER: Extension template (do not remove this comment)
with contextlib.suppress(ImportError):
    import yam_ultra_deformable_place.tasks_experimental  # noqa: F401
...
def _format_presets(preset_map: dict | None) -> str:
    ...
    Args:
        preset_map: Mapping of :class:`~yam_ultra_deformable_place.utils.preset_target.PresetTarget`
    ...
    if preset_map is None:
        return "(unavailable)"
    from yam_ultra_deformable_place.utils.preset_target import PresetTarget
...
def main():
    """Print all environments registered in `yam_ultra_deformable_place` extension."""
    ...
    if args_cli.show_presets:
        from yam_ultra_deformable_place.utils.preset_cli import enumerate_task_presets
